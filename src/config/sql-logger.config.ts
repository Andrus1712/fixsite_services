...
export class SqlLogger implements Logger {
  ...
  constructor() {
    ...
    this.logger = winston.createLogger({
      level: 'info',
      format: winston.format.printf(({ timestamp, level, message, query, parameters, error, time }) => {
        const params = parameters && Array.isArray(parameters) && parameters.length > 0 ? ` | Parameters: ${JSON.stringify(parameters)}` : '';
